...
fn calculate(input: &String) -> Result<String, String> {
    let mut num_of_operators  = 0; 
    for char in input.chars() {
        if char == '+' || char == '-' || char == '*' || char == '/' || char == '^' || char == '%' || char == '&' {
            num_of_operators += 1;
        }
        if char.is_alphabetic() || char.is_whitespace() {
            return Err(format!("Invalid input: {} | Invalid character {}", input, char));
        }
    }
    if num_of_operators != 1 {
        return Err(format!("Invalid input: {} | Num of operators {}", input, num_of_operators));
    }
    let re_nums = Regex::new(r"((\d*\.\d*)|(\d*))").unwrap();
    let re_op = Regex::new(r"[+*\-/^%&]");
    let captures_nums: Vec<Captures> = re_nums.captures_iter(input).collect();
    let captures_op = re_op.unwrap().captures(input).unwrap();
    
    if captures_op.len() != 1 || captures_nums.len() != 2 {
        return Err(format!("Invalid input: {}", input));
    }
    
    let num1 = captures_nums[0].get(0).unwrap().as_str().parse::<f64>().unwrap();
    let num2 = captures_nums[1].get(0).unwrap().as_str().parse::<f64>().unwrap();
    let op = captures_op.get(0).unwrap().as_str();
    
    if op == "+" {
        return Ok(format!("{} = {}", input, num1 + num2));
    } else if op == "-" {
        return Ok(format!("{} = {}", input, num1 - num2));
    } else if op == "*" {
        return Ok(format!("{} = {}", input, num1 * num2));
        ...
        if num2 == 0.0 {
            return Err(format!("Invalid input: {} | Division by zero", input));
        }
        return Ok(format!("{} = {}", input, num1 / num2));
    } else if op == "^" {
        return Ok(format!("{} = {}", input, num1.powf(num2)));
    } else if op == "%" {
        return Ok(format!("{} = {}", input, num1 % num2));
    } else if op == "&" {
        return Ok(format!("{} = {}", input, num1 as i32 & num2 as i32));
    }
    
    
    return Ok(format!("{} = {}", input, 0));
}
...
impl App {
    ...
    fn submit_message(&mut self) {
        let result = calculate(&self.input);
        match result {
            Ok(res) => {
